...
const MAX_SE = 5;
const DECAY_TIME = 6.0;
const DR_PER = 0.03;
const MANA_PER_HIT = 5;
const MANA_RATIO = 0.08;

function se(p: Player) {
  if (!p.magicSwordsman) p.magicSwordsman = { swordEnergy: 5, decayTimer: 0, enhanced: false, enhancedTimer: 0 };
...
registerTalent('arcane_contract', {
  ...
  canCast(_state, p, slot, _talent) {
    if (slot === 'skill1' && se(p).swordEnergy < 2) return false;
    if (slot === 'skill2' && se(p).swordEnergy < 3) return false;
    if (slot === 'ultimate' && se(p).swordEnergy < 5) return false;
    return true;
  },

  // 魔力比例轉化為額外魔法傷害 + 魔刃強化 / 劍氣波強化
  ...
  onCastResolved(state, p, _action, slot, _talent) {
    const s = se(p);
    if (slot === 'skill1' && s.swordEnergy >= 2) {
      s.swordEnergy -= 2;
      s.decayTimer = 0;
    ...
    if (slot === 'skill2' && s.swordEnergy >= 3) {
      s.swordEnergy -= 3;
      s.decayTimer = 0;
      s.enhanced = true;
      s.enhancedTimer = 5.0;
...
export function tickMagicSwordsman(state: any, p: Player, dt: number) {
  ...
  if (s.swordEnergy > 0) {
    s.decayTimer += dt;
    while (s.decayTimer >= DECAY_TIME && s.swordEnergy > 0) {
      s.decayTimer -= DECAY_TIME;
      s.swordEnergy -= 1;
