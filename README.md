...
以 **three.js (WebGL)** 渲染、**React + Vite + TypeScript** 打造的 3D（俯視傾斜鏡頭）多人連線魔王遠征遊戲。
與夥伴**組隊遠征、輪番討伐各路魔王**，逐關推進、**征服全部關卡即為通關**（亦保留全員混戰的競技玩法）。
...
- 🐲 **魔王遠征**：多回合連續討伐，魔王機制由淺入深，全員協力闖關
- 👥 最多 8 人組隊遠征（亦可全員混戰）
- 🦸 **21 種角色**，各有強項與弱項、**各自獨特的 3D 技能特效與專屬大絕招**
...
| 1 | 🔵 法師 | 後排·爆發 | 冰矛/燃燒/流星 |
| 2 | 🟣 刺客 | 近戰·疊毒 | 疊毒續戰、瘟疫爆發、隱身 |
| 3 | ⬜ 坦克 | 前排·保護 | 怒氣壁壘：越打越強、躍擊開團、全隊護罩 |
| 4 | 🟢 弓箭手 | 後排·持續 | 越遠越痛、寄生孵化引爆、天羽箭暴連射 |
| 5 | ⚪ 治療師 | 支援·核心 | 全隊回血、逆轉 |
| 6 | 🔴 狂戰士 | 突進·處決 | 殘血越強、處決 |
| 7 | 🖤 忍者 | 機動·控制 | 影縛處決、千影分身 |
| 8 | 🟠 元素使 | 控制·封鎖 | 火牆/冰徑/隕石 |
| 9 | 🟡 格鬥家 | 近戰·蓄氣爆發 | 武僧：聚氣強身、不動明王金身、化龍真·昇龍霸 |
...
| 18 | 💫 星環使 | 遠程·星砲 | 環繞星球、星軌砲 |
| 19 | 🦅 鳥獵 | 後排·攻速 | 鷹瞳爆擊、鷹擊風暴 |
| 20 | 🗡️ 魔劍士 | 近戰·魔武 | 雙資源、劍氣強化 |
...
src/
  ...
  game/               命令式遊戲引擎（核心邏輯已型別化為 .ts；render3d 程序建模仍用 .js）
    ...
    characters.js     → characters/ 桶（21 角色 + 負數 minion 模板）
    ...
    render3d/         three.js 渲染模組
      ...
      models.js       21 角色程序化 3D 模型 + 動畫狀態機
      ...
      coords.js       世界座標 ↔ 場景座標
      classes/        各角色的程序化建模；vfx/  每角色專屬技能特效（registry + 共用 lib）
